...
Several philosophers are sitting around a table either eating, thinking, or sleeping. On the table is a large bowl of spaghetti, and there is a fork between each philosopher. To eat they must pick up two forks (on their left and right), and they will set down the forks when they have finished eating. Each philosopher must try to eat again soon enough to avoid dying. They do not speak to each other, and they do not know when another philosopher is hungry.

In the program, each philosopher is represented by a POSIX thread and each fork is a mutex (to ensure it only be used by one thread at a time). The simulation parameters are defined by the following arguments to the program.
...
The case of five philosophers is similar, except the first four can eat in pairs followed by number 5 eating alone.
